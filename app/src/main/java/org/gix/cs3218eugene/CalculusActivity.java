...
//        --- missing codes -----
    ...
    private double integrateByTrapezium()
    {
        ...
        // Sum of all Trapezoid, mathematically the formula can be simplified into the following
        // area = 0.5 height (f(0) + 2(f(1) + f(2) + ... + f(n-2)) + fn-1)
        double mid_sum = 0;
        for (int i = 1; i < bufferSize - 1; i++){
            mid_sum += productSig_double[i];
        }
        double area = 0.5 * deltaTime * ( productSig_double[0] + 2 * mid_sum + productSig_double[bufferSize-1]);
        return area;
    }
